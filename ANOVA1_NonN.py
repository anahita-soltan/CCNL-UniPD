...
save_path = r'C:\Users\Admin\Documents\Mistry\data\Analysis\ANOVA\non_normalized\New\ANOVA1'
                ...
                for i in range(num_pixels_i):
                    for j in range(num_pixels_j):
                        ...
                        activations_by_number = [
                            activations[label_data == num] for num in range(9)
                        ]
                        f_statistic, p_value = f_oneway(*activations_by_number)
                        
                        if p_value < 0.01:
                            # Find the number with the highest mean activation
                            mean_activations = [np.mean(acts) for acts in activations_by_number]
                            max_index = mean_activations.index(max(mean_activations))
                            significant_neurons.append({
                                'Channel': channel,
                                'Kernel': kernel,
                                'Neuron_i': i,
                                'Neuron_j': j,
                                'Tuned_Number': max_index,
